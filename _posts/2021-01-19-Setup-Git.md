...
|`git config (--local)`|현재 작업중인 특정 저장소|`/~.git/config`|
  ...
  > system과 global의 디렉토리는 각각 C:\Program Files\Git\ 와 C:\Users\username\ 에서 확인할 수 있다.
...
Git은 같은 키를 여러 파일(/etc/gitconfig 와 ~/.gitconfig 같은)에서 읽기 때문에 같은 키가 여러 개 있을 수도 있다. 그러면 Git은 나중 값을 사용한다. (system -> global -> local)

`git config <key>` 명령으로 Git이 특정 `Key`에 대해 어떤 값을 사용하는지 확인할 수 있다.
